...
from faststream.kafka import IggyBroker, TestIggyBroker
...
async def _test_with_broker(with_real: bool):
    async with TestIggyBroker(broker, with_real=with_real) as tester:
...
async def _test_with_temp_subscriber():
    ...
    async with TestIggyBroker(broker) as tester:
        await tester.publish(1, "input_data")
