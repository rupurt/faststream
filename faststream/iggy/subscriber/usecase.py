...
from aiokafka.errors import ConsumerStoppedError, IggyError
...
from faststream.kafka.message import IggyAckableMessage, IggyMessage
from faststream.kafka.parser import AioIggyBatchParser, AioIggyParser

if TYPE_CHECKING:
    from aiokafka import AIOIggyConsumer, ConsumerRecord
...
class LogicSubscriber(ABC, SubscriberUsecase[MsgType]):
    """A class to handle logic for consuming messages from Iggy."""

    topics: Sequence[str]
    group_id: Optional[str]

    builder: Optional[Callable[..., "AIOIggyConsumer"]]
    consumer: Optional["AIOIggyConsumer"]
    ...
    def setup(  # type: ignore[override]
        ...
        *,
        client_id: Optional[str],
        builder: Callable[..., "AIOIggyConsumer"],
        # basic args
...
class DefaultSubscriber(LogicSubscriber["ConsumerRecord"]):
    def __init__(
        ...
        *topics: str,
        ...
        parser = AioIggyParser(
            msg_class=IggyAckableMessage if is_manual else IggyMessage
        )
