...
from faststream.kafka.broker.logging import IggyLoggingBroker
...
if TYPE_CHECKING:
    ...
    class IggyInitKwargs(TypedDict, total=False):
...
class IggyBroker(
    ...
    url: List[str]
    _producer: Optional["AioIggyFastProducer"]
    ...
    async def connect(  # type: ignore[override]
        ...
        bootstrap_servers: Annotated[
            ...
            Doc("Iggy addresses to connect."),
        ] = Parameter.empty,
        **kwargs: "Unpack[IggyInitKwargs]",
    ) -> Callable[..., aiokafka.AIOIggyConsumer]:
        """Connect to Iggy servers manually.

        Consumes the same with `IggyBroker.__init__` arguments and overrides them.
    ...
    async def _connect(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        producer = aiokafka.AIOIggyProducer(
            **kwargs,
            client_id=client_id,
        )

        await producer.start()
        self._producer = AioIggyFastProducer(
            producer=producer,
        )

        return partial(
            aiokafka.AIOIggyConsumer,
            **filter_by_dict(ConsumerConnectionParams, kwargs),
        )

    async def start(self) -> None:
        """Connect broker to Iggy and startup all subscribers."""
        await super().start()
