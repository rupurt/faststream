...
from faststream.kafka.message import FAKE_CONSUMER, IggyMessage
...
class AioIggyParser:
    """A class to parse Iggy messages."""

    def __init__(self, msg_class: Type[IggyMessage]) -> None:
...
class AioIggyBatchParser(AioIggyParser):
    async def parse_message(
        self,
        message: Tuple["ConsumerRecord", ...],
    ) -> "StreamMessage[Tuple[ConsumerRecord, ...]]":
        """Parses a batch of messages from a Iggy consumer."""
        body: List[Any] = []
    ...
    async def decode_message(
        ...
        return [
            decode_message(await super(AioIggyBatchParser, self).parse_message(m))
